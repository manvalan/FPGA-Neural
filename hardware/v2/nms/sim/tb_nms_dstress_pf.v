...
module tb #(
        ...
        for (gi = 0; gi < N_SLOTS_CFG; gi = gi + 1) begin : GEN_SLOT_PF_MON
            ...
            always @(*) begin
                ...
                slot_weight_blocking[gi] =
                    ...
                    !u_nmp.u_dataflow_core.GEN_SLOT[gi].u_mm.read_issued &&
                    !u_nmp.u_dataflow_core.GEN_SLOT[gi].u_mm.read_ready;
            end
        end
    endgenerate
    ...
    initial begin
        ...
        $display("========================================");
        $display("NMS D-Stress benchmark (STEP11, REAL weight prefetch, real V1 PSRAM chain) -- N_SLOTS_CFG=%0d PFD_CFG=%0d", N_SLOTS_CFG, PFD_CFG);
        $display("========================================");
        ...
        $display("========================================");
        if (errors == 0)
            $display("ALL %0d WORKLOAD SUITES PASSED (N_SLOTS_CFG=%0d, PFD_CFG=%0d, real V1 PSRAM chain, real slot_mem_arbiter, REAL weight_prefetch_engine)", tests, N_SLOTS_CFG, PFD_CFG);
        else
