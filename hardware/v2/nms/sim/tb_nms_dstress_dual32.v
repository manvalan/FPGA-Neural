...
module tb #(
    ...
    // STEP15 (continuation): the real dual-chip 32-bit weight-fetch
    // physical interface, in ADDITION to the original single-chip
    // psram_a/psram_dq above (which now serves ONLY activation-fetch
    // and result-write-back).
    wire [ADDR_WIDTH-1:0]  psram01_a;
    wire [15:0]            psram0_dq, psram1_dq;
    wire psram01_ce_n, psram01_oe_n, psram01_we_n, psram01_lb_n, psram01_ub_n, psram01_zz_n;

    nms_neural_multiprocessor_dual32 #(
        ...
        .MAX_TILES(16), .PREFETCH_DISTANCE(PFD_CFG), .PSRAM_DATA_WIDTH(PSRAM_DATA_WIDTH), .CLK_FREQ_MHZ(80)
    ) u_nmp (
        .clk(clk), .rst(rst),
        ...
        .psram01_a(psram01_a),
        .psram01_ce_n(psram01_ce_n), .psram01_oe_n(psram01_oe_n), .psram01_we_n(psram01_we_n),
        .psram01_lb_n(psram01_lb_n), .psram01_ub_n(psram01_ub_n), .psram01_zz_n(psram01_zz_n),
        .psram0_dq(psram0_dq), .psram1_dq(psram1_dq)
    );

    psram_model #(.ADDR_WIDTH(ADDR_WIDTH), .DATA_WIDTH(16), .DEPTH(524288)) u_psram0 (
        .clk(clk), .a(psram01_a), .dq(psram0_dq),
        .ce_n(psram01_ce_n), .oe_n(psram01_oe_n), .we_n(psram01_we_n),
        .lb_n(psram01_lb_n), .ub_n(psram01_ub_n), .zz_n(psram01_zz_n)
    );
    psram_model #(.ADDR_WIDTH(ADDR_WIDTH), .DATA_WIDTH(16), .DEPTH(524288)) u_psram1 (
        .clk(clk), .a(psram01_a), .dq(psram1_dq),
        .ce_n(psram01_ce_n), .oe_n(psram01_oe_n), .we_n(psram01_we_n),
        .lb_n(psram01_lb_n), .ub_n(psram01_ub_n), .zz_n(psram01_zz_n)
    );
    ...
    // STEP15 (continuation): WEIGHT data now lives on the real dual-
    // chip 32-bit physical interface (psram0/psram1), not the single
    // psram_model above (which now serves ONLY activation+result-
    // writeback). Same byte-lane mapping as psram_controller_dual32.v
    // itself: 32-bit word W = byte_addr>>2; byte_addr[1:0] selects
    // which of chip0's low/high byte or chip1's low/high byte.
    ...
    function automatic signed [7:0] peek_byte_weight(input [ADDR_WIDTH-1:0] byte_addr);
        ...
        begin
            word32_addr = byte_addr >> 2;
            case (byte_addr[1:0])
                2'd0: peek_byte_weight = u_psram0.mem[word32_addr][7:0];
                2'd1: peek_byte_weight = u_psram0.mem[word32_addr][15:8];
                2'd2: peek_byte_weight = u_psram1.mem[word32_addr][7:0];
                2'd3: peek_byte_weight = u_psram1.mem[word32_addr][15:8];
            endcase
        end
    endfunction
    ...
    initial begin
        ...
        $display("========================================");
        $display("NMS D-Stress benchmark (STEP15 continuation, REAL DUAL-CHIP 32-BIT weight path, real V1 PSRAM for activation+writeback) -- N_SLOTS_CFG=%0d PFD_CFG=%0d", N_SLOTS_CFG, PFD_CFG);
        $display("========================================");

        wait (u_nmp.u_psram_ctrl.state == u_nmp.u_psram_ctrl.STATE_IDLE);
        wait (u_nmp.u_psram_dual32.u_ctrl0.state == u_nmp.u_psram_dual32.u_ctrl0.STATE_IDLE);
        wait (u_nmp.u_psram_dual32.u_ctrl1.state == u_nmp.u_psram_dual32.u_ctrl1.STATE_IDLE);
        @(posedge clk);
        ...
        $display("========================================");
        if (errors == 0)
            $display("ALL %0d WORKLOAD SUITES PASSED (N_SLOTS_CFG=%0d, PFD_CFG=%0d, REAL dual-chip 32-bit weight path, real V1 PSRAM for activation+writeback)", tests, N_SLOTS_CFG, PFD_CFG);
        else
