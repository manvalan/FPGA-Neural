...
module tb #(
    ...
    // STEP16 Phase 5: the real single-chip SDRAM weight-fetch physical
    // interface, in ADDITION to the original single-chip psram_a/
    // psram_dq above (which still serves ONLY activation-fetch and
    // result-write-back, byte-for-byte unchanged from the dual32
    // baseline).
    ...
    nms_neural_multiprocessor_sdram_pack128 #(
    ...
    // STEP16 Phase 5: WEIGHT data now lives on the real single-chip
    // SDRAM physical interface (u_sdram), not the single psram_model
    // above (which still serves ONLY activation+result-writeback).
...
`endif
    ...
    initial begin
        ...
        $display("========================================");
        $display("NMS D-Stress benchmark (STEP16 Phase 5, REAL SDRAM (AS4C4M16SA-6TIN) weight path, real V1 PSRAM for activation+writeback) -- N_SLOTS_CFG=%0d PFD_CFG=%0d", N_SLOTS_CFG, PFD_CFG);
        $display("========================================");
        ...
        $display("========================================");
        if (errors == 0)
            $display("ALL %0d WORKLOAD SUITES PASSED (N_SLOTS_CFG=%0d, PFD_CFG=%0d, REAL SDRAM weight path, real V1 PSRAM for activation+writeback)", tests, N_SLOTS_CFG, PFD_CFG);
        else
